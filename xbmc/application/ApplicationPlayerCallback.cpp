...
void CApplicationPlayerCallback::OnPlayBackStarted(const CFileItem& file)
{
  ...
  if (stackHelper->IsPlayingISOStack() || stackHelper->IsPlayingRegularStack())
    itemCurrentFile.reset(new CFileItem(*stackHelper->GetRegisteredStack(file)));
  else
    itemCurrentFile.reset(new CFileItem(file));
  ...
  CServiceBroker::GetPVRManager().OnPlaybackStarted(*itemCurrentFile);
