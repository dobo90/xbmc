...
bool CWinSystemGbmEGLContext::InitWindowSystemEGL(EGLint renderableType, EGLint apiType)
{
  ...
  if (CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_forceDisableAsyncRendering)
  {
    CLog::Log(LOGINFO, "[GBM] async rendering disabled");
  }
  else if (CEGLUtils::HasExtension(m_eglContext.GetEGLDisplay(), "EGL_ANDROID_native_fence_sync") &&
